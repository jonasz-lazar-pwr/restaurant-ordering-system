...
class PayUClient:
    def __init__(self):
        self.token_url = f"{config.PAYU_SANDBOX_URL}pl/standard/user/oauth/authorize"
    ...
    def get_payment_methods(self):
        url = f"{config.PAYU_SANDBOX_URL}api/v2_1/paymethods"
    ...
    def create_order(self, order_data):
        url = f"{config.PAYU_SANDBOX_URL}api/v2_1/orders"
    ...
    def get_order_status(self, order_id):
        url = f"{config.PAYU_SANDBOX_URL}api/v2_1/orders/{order_id}"
        response = self.session.get(url, headers=self._auth_headers())
        response.raise_for_status()  # Check for HTTP error
    ...
    def refund_order(self, order_id, refund_data):
        url = f"{config.PAYU_SANDBOX_URL}api/v2_1/orders/{order_id}/refunds"
